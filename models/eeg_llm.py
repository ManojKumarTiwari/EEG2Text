...
class EEGTokenReducer(nn.Module):
    ...
    def __init__(self, area_config, temporal_pool_stride=2):
        super().__init__()
...
class EEGLanguageModel(nn.Module):
    ...
    def __init__(self, param):
        ...
        # ---- FACED brain region config (from model_for_faced.py) ----
        FACED_brain_regions = [
        ...
        # Compute sorted indices (same logic as model_for_faced.py)
        FACED_region_groups = {}
        ...
        for region in sorted(FACED_region_groups.keys()):
            ...
            sorted_electrodes = sorted(
                region_electrodes, key=lambda x: FACED_topology[region].index(x[1])
            )
    ...
    def forward(self, eeg_data, prompt_ids, prompt_mask, target_ids, target_mask):
        ...
        with torch.no_grad():
            ...
            with torch.amp.autocast('cuda', enabled=False):
                eeg_features = self.eeg_encoder(eeg_data[:, :30, :, :].float())
            # (batch, 30, 30, 200)
